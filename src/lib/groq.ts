...
const LEGAL_RESPONSES: ResponseRule[] = [
  { keywords: ['texas', 'tx bar', 'bar association', 'rule 1.01', 'competence', 'malpractice'], response: "State Bar of Texas Rule 1.01 on Competent and Diligent Representation now explicitly includes understanding AI tools used in legal practice. Your Alignment Certificate references this rule directly — documenting that LexAI was trained and verified by a TX Bar licensed attorney. This is the compliance trail the bar association looks for." },
  { keywords: ['non-compete', '15.50', 'compete', 'employment'], response: "Texas Business and Commerce Code Section 15.50 allows courts to reform overly broad non-competes. Unlike some states, Texas does enforce reasonable non-competes — but they must be ancillary to an otherwise enforceable agreement. LexAI is calibrated to cite this statute specifically rather than giving generic multi-jurisdiction hedging." },
  { keywords: ['deposit', 'landlord', 'tenant', 'rental', 'security deposit'], response: "Texas Property Code Section 92.103 gives landlords 30 days to return a security deposit or provide an itemized statement. LexAI knows this timeline and cites it specifically rather than saying laws vary by jurisdiction." },
  { keywords: ['record', 'recording', 'consent', 'wiretap', 'penal code'], response: "Texas Penal Code Section 16.02 makes Texas a one-party consent state for recording conversations. LexAI is calibrated to cite this directly rather than hedging with generic consent language when the client is clearly in Texas." },
