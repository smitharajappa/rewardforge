  ...
  if (done) {
    ...
    return (
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="w-[72px] h-[72px]"><svg viewBox="0 0 72 72"><polygon points="36,4 66,20 66,52 36,68 6,52 6,20" fill="#071020" stroke="#38bdf8" strokeWidth="2.5"/><polyline points="14,52 26,40 36,44 46,28 58,16" fill="none" stroke="#38bdf8" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/><circle cx="58" cy="16" r="4" fill="#38bdf8"/></svg></div>
        ...
        <div className="flex items-center gap-2">
          {[`${comparisons.length} Total`, `${countA} Prefer A`, `${countB} Prefer B`, `${countTie} Ties`].map(s => (
            <span key={s} className="font-mono text-[10px] px-3 py-1 rounded-full" style={{ background: '#0a0a0a', border: '1px solid #1a1a1a', color: '#a3a3a3' }}>{s}</span>
          ))}
        </div>
        <div className="flex gap-3">
          <button onClick={() => navigate('/evaluate')} className="px-5 py-2.5 rounded-full font-syne font-bold text-sm" style={{ background: '#fafafa', color: '#000' }}>View in Evaluate</button>
          <button onClick={() => { setStep(0); setSelection(null); setReasoning(''); setDone(false); }} className="px-5 py-2.5 rounded-full font-syne font-bold text-sm" style={{ border: '1px solid #1a1a1a', color: '#fafafa' }}>Restart</button>
        </div>
      </motion.div>
    );
  }
    ...
    <div className="space-y-5 max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-2 gap-4">
        {(['A', 'B'] as const).map(id => {
          ...
          return (
            <div key={id} onClick={() => handleSelect(id)} style={{ background: '#0a0a0a', border: `2px solid ${isSelected ? color : '#1a1a1a'}`, boxShadow: isSelected ? `0 0 0 3px ${color}14` : 'none', borderRadius: 10, padding: '14px', cursor: 'pointer', height: 280, display: 'flex', flexDirection: 'column', transition: 'all 0.12s' }}>
              <div className="flex justify-between items-center mb-3">
                <span className="font-mono text-[10px] px-2 py-1 rounded" style={{ background: `${color}18`, border: `1px solid ${color}30`, color }}>RESPONSE {id}</span>
                {isSelected && <CheckCircle2 size={16} style={{ color }} />}
              </div>
              <div className="flex-1 overflow-y-auto text-[13px] leading-relaxed pr-1 transition-colors" style={{ color: isSelected ? '#fafafa' : '#a3a3a3' }}>
                <pre className="whitespace-pre-wrap font-syne">{content}</pre>
              </div>
              <div className="mt-3 pt-3 text-center font-mono text-[10px] uppercase tracking-widest" style={{ borderTop: '1px solid #1a1a1a', color: '#333' }}>
                Click or press <kbd className="px-1.5 py-0.5 rounded text-[9px]" style={{ background: '#111', border: '1px solid #1a1a1a', color: '#525252' }}>{id}</kbd>
              </div>
            </div>
          );
        })}
      ...
      <div className="flex justify-center gap-3">
        {['Tie', 'Skip'].map(action => (
          <button key={action} onClick={() => action === 'Tie' ? handleSelect('Tie') : (setStep(s => Math.min(s + 1, TOTAL - 1)), setSelection(null), setReasoning(''))}
            className="px-5 py-2 rounded-lg font-syne font-bold text-sm flex items-center gap-2 transition-all"
            style={{ background: selection === 'Tie' && action === 'Tie' ? '#fafafa' : 'transparent', color: selection === 'Tie' && action === 'Tie' ? '#000' : '#525252', border: `1px solid ${selection === 'Tie' && action === 'Tie' ? '#fafafa' : '#1a1a1a'}` }}>
            {action} <kbd className="rounded text-[10px] px-1.5 py-0.5" style={{ background: '#111', border: '1px solid #1a1a1a', color: '#525252' }}>{action === 'Tie' ? 'T' : 'S'}</kbd>
          </button>
        ))}
      </div>
      ...
      <AnimatePresence>
        {selection && (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }}
            ...
            <textarea value={reasoning} onChange={e => setReasoning(e.target.value)} rows={3} resize-none
...
function RateTab() {
  ...
  const handleSubmit = () => {
    addRating({ id: crypto.randomUUID(), prompt: PROMPTS[promptIdx].text, ...scores as any, overall, timestamp: new Date() });
  ...
  const qualifier = overall >= 8 ? { label: 'Excellent', color: '#34d399' } : overall >= 5 ? { label: 'Good', color: '#38bdf8' } : { label: 'Needs work', color: '#f472b6' };
      ...
      <div className="p-5 rounded-xl space-y-4" style={{ background: '#0a0a0a', border: '1px solid #1a1a1a' }}>
        {DIMENSIONS.map(d => (
          <div key={d.key} className="flex items-center gap-4">
            ...
            <span className="font-mono text-[9px] px-1.5 py-0.5 rounded shrink-0" style={{ background: '#111', border: '1px solid #1a1a1a', color: '#525252' }}>
                ...
                return (
                  <tr key={r.id} style={{ borderBottom: i < ratings.length - 1 ? '1px solid #0f0f0f' : 'none' }}>
                    ...
                    {['helpfulness', 'accuracy', 'safety', 'coherence', 'creativity'].map(k => (
                      <td key={k} className="px-3 py-2 font-mono text-[10px]" style={{ color: '#525252' }}>{(r as any)[k].toFixed(1)}</td>
                    ))}
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm mt-1" style={{ color: '#525252' }}>Collect human preference data to train your reward model.</p>
      </div>
      ...
      <AnimatePresence mode="wait">
        {tab === 'pairwise' ? <motion.div key="pairwise" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><PairwiseTab /></motion.div>
          : <motion.div key="rate" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><RateTab /></motion.div>}
      </AnimatePresence>
    </div>
  );
}
